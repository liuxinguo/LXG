...
<form action="<?php echo U('djrw_list');?>" name="Form2" method="post">
          <div class="user_message fr"><i class="icon-wrench"></i>
		  <a class="btn btn-primary btn-small" href="<?php echo U('djrw_add',array('gid'=>$gid));?>">添加接口</a>
		  </div>
		  </form>
          <div class="cl"></div>
        </div>

  <form action="<?php echo U('djrw_del');?>" method="post" name="form1">
...
<thead>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';">    
   <td class="table-textcenter"><input type="checkbox" name="duoxuan" value="<?php echo ($vo["id"]); ?>"></td>
    ...
    <td class="table-textcenter"><?php echo ($vo["dj"]); ?></td>
    <td class="table-textcenter"><?php if($vo["sjlb"] == 1 ): echo ($vo["sj"]); ?>小时<?php else: echo ($vo["sj"]); ?>天<?php endif; ?> </td>
    <td class="table-textcenter"><?php echo ($vo["yb"]); ?></td>
    <td <?php if($vo["lb"] == 1 ): ?>class="table-textcenter red">元宝<?php else: ?>class="table-textcenter">平台币<?php endif; ?></td>
    <td class="table-textcenter"><a href="<?php echo U('djrw_edit',array('id'=>$vo['id']));?>">修改</a></td>
