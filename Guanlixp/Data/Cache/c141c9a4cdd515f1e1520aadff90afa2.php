...
function gotoserver(){
  ...
  <table class="table set_table">
    <form action="<?php echo U('pay_fall',array('begintime'=>$Think['begintime'],'endtime'=>$Think['endtime']));?>" name="Form2" method="post">
    <tr>
...
<thead>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';" class="tabletr">    
    ...
    <td class="table-textcenter"><a href="<?php echo U('updateorder',array('paysn'=>$vo['paysn'],'paytable'=>'game_ok'));?>"><?php echo ($vo["paysn"]); ?></a></td>
    ...
    <td align="center"><a href="<?php echo U('pay/bdpay',array('paysn'=>$vo['paysn']));?>" style="color:#1b10f5;" target="_blank">补单</a></td>
