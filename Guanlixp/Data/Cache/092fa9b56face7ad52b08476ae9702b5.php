...
function gotoserver(){
  ...
  <table class="table set_table">
    <form action="<?php echo U('pay_reissue',array('begintime'=>$Think['begintime'],'endtime'=>$Think['endtime']));?>" name="Form2" method="post">
    <tr>
    ...
    <tr>
        <td height="30" width="200" class="table-textcenter">操作员：</td>
        <td class="huise_font">
            <select name="operater" id="operater">
                <option value="0">请选择操作员</option>
                <?php if(is_array($operater)): $i = 0; $__LIST__ = $operater;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$operater): $mod = ($i % 2 );++$i;?><option value='<?php echo ($operater["operater"]); ?>' <?php if($operater["operater"] == $get['operater']): ?>selected='selected'<?php endif; ?>><?php echo ($operater["operater"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </td>
    </tr>
    <tr>
...
<thead>
  <tr>
    ...
    <th width="6%" class="table-textcenter">充值方式</th>
    <th width="6%" class="table-textcenter">银行类型</th>
    ...
    <th width="6%" class="table-textcenter">操作员</th>
  </tr>
  </thead>
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';" class="tabletr">    
    ...
    <td class="table-textcenter"><?php echo ($vo["paybank"]); ?></td>
