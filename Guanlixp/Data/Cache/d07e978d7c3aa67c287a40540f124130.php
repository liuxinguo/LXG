...
function gotoserver(){
  ...
  <table class="table set_table">
    <form action="<?php echo U('cps/qudao_sc',array('begintime'=>$Think['begintime'],'endtime'=>$Think['endtime']));?>" name="Form2" method="post">
    <tr>
    ...
    <tr>
        <td height="30" width="200" class="table-textcenter">充值站点：</td>
        <td class="huise_font">
            <select name="mid" id="mid">
				<option value='0' <?php if($paygame["id"] == 0): ?>selected='selected'<?php endif; ?>>充值渠道</option>
			  
                <?php if(is_array($hunuser)): $i = 0; $__LIST__ = $hunuser;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["mid"]); ?>' <?php if($vo["mid"] == $get['mid']): ?>selected='selected'<?php endif; ?>><?php echo ($vo["mname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
...
<thead>
  <tr>
    ...
    <th class="table-textcenter">充值渠道</th>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';">    
    ...
    <td class="table-textcenter"><?php if(($vo['process']) == "1"): ?><font color="red"><b>已成功</b></font><?php else: ?><a >失败</a><?php endif; ?></td>
