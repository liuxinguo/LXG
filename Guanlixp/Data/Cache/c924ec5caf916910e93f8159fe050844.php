...
<body>
      <div class="content">
        ...
        <div class="minbox">
       <form name="newsform" method="post" action="<?php echo U('Manage_del');?>">
        	...
        	<table class="table table-bordered table-striped">
               ...
               <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rs): $mod = ($i % 2 );++$i;?><tr>
                		<td align="center" class="table-textcenter"><input type="checkbox" name="duoxuan" value="<?php echo ($rs["id"]); ?>" ></td>
                        <td class="table-textcenter" ><?php echo ($rs["adminid"]); ?></td>
						<td class="table-textcenter" ><?php echo ($rs["adminname"]); ?></td>
						<td class="table-textcenter"><?php echo ($rs["name"]); ?></td>
                        <td class="table-textcenter"><?php echo ($rs["loginip"]); ?></td>
                        <td class="table-textcenter"><?php echo (date("Y.n.j H:i:s",$rs["lastlogintime"])); ?></td>
                        <td class="table-textcenter table_config"><a href="<?php echo U('manage_edit',array('id'=>$rs['adminid']));?>">修改</a>  <a href="__URL__/del/id/<?php echo ($rs["adminid"]); ?>">删除</a></td>
