...
function gotoserver(){
  ...
  <table class="table set_table">
    ...
    <form action="<?php echo U('pay_record_log',array('begintime'=>$Think['begintime'],'endtime'=>$Think['endtime']));?>" name="Form2" method="post">
    <tr>
...
<thead>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';">    
    ...
    <td class="table-textcenter"><?php if(($vo['process']) == "1,1,1"): ?><font color="red"><b>成功</b></font><?php else: ?><a href="<?php echo U('pay/bdgamepay',array('paysn'=>$vo['paysn']));?>" style="color:#1b10f5;" target="_blank">补单</a><?php endif; ?></td>
