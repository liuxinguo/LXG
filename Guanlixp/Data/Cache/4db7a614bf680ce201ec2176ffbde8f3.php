...
function gotoserver(){
  ...
  <table class="table set_table">
    <form action="<?php echo U('pay_game_record',array('begintime'=>$Think['begintime'],'endtime'=>$Think['endtime']));?>" name="Form2" method="post">
    <tr>
    ...
    <tr>
        ...
        <td class="huise_font">
            <select name="unid" id="unid">
				<option value='0' selected='selected'>充值站点</option>
			  
                <?php if(is_array($qdlist)): $i = 0; $__LIST__ = $qdlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["mid"]); ?>' <?php if($vo["mid"] == $get['unid']): ?>sele cted='selected'<?php endif; ?>><?php echo ($vo["mname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
...
<thead>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';">    
    ...
    <td class="table-textcenter"><a href="<?php echo U('admin/updateorder',array('paysn'=>$vo['paysn'],'paytable'=>'game_togame'));?>" title="更新订单状态"><?php echo ($vo["paysn"]); ?></a></td>
