...
<form action="<?php echo U('article');?>" name="Form2" method="post">
...
<thead>
   ...
   <a href="<?php echo U('article');?>"  class="ona">全部</a>
   <a href="<?php echo U('article',array('cid'=>1));?>" >新闻公告</a>
   <a href="<?php echo U('article',array('cid'=>2));?>" >游戏公告</a>
   <a href="<?php echo U('article',array('cid'=>3));?>" >活动资讯</a>
   <a href="<?php echo U('article',array('cid'=>4));?>" >综合新闻</a>
   <a href="<?php echo U('article',array('cid'=>5));?>" >充值问题</a>
   <a href="<?php echo U('article',array('cid'=>6));?>" >游戏问题</a>
   <a href="<?php echo U('article',array('cid'=>7));?>" >账号问题</a>
   <a href="<?php echo U('article',array('cid'=>8));?>" >热门活动专题</a>
   <a href="<?php echo U('article',array('cid'=>9));?>" >游戏盒专区</a>
   <a href="<?php echo U('article',array('cid'=>10));?>" >安全问题</a>
   </th>
   </tr>
</TABLE>
  <form name="form1" method="post" action="<?php echo U('article_del');?>">
...
<thead>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr onMouseOver="this.className='mouseon'" onMouseOut="this.className='';">    
    ...
    <td align="center" nowrap><a href="<?php echo U('article/view',array('id'=>$vo['id']));?>" title="$vo.title" target="_blank"><font color="<?php echo ($vo["color"]); ?>"><?php echo (mysubstr('0','18',$vo["title"])); ?></font></a></td>
    <td class="table-textcenter"><?php echo ($vo["cid"]); ?></td>
    <td class="table-textcenter"><?php if(!empty($vo['pic'])): ?><a href="<?php echo ($vo["pic"]); ?>" target="_blank">查看图片</a><?php endif; ?></td>
    <td class="table-textcenter"><?php if(!empty($vo['thumbpic'])): ?><a href="<?php echo ($vo["thumbpic"]); ?>" target="_blank">查看图片</a><?php endif; ?></td>
    <td nowrap class="table-textcenter"><?php if(($vo['url']) == "http://"): else: ?><a href="<?php echo ($vo["url"]); ?>" target="_blank">查看链接</a><?php endif; ?></td>
    <td class="table-textcenter"><?php echo ($vo["flag"]); ?></td>
    <td class="table-textcenter"><?php echo ($vo["clicknum"]); ?></td>
    ...
    <td class="table-textcenter"><a href="<?php echo U('article_edit',array('id'=>$vo['id']));?>">修改</a></td>
