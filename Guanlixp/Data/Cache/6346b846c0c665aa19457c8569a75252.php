...
function gotoserver(){
  ...
  <table class="table set_table">
    <form action="<?php echo U('Hunfu/pay_game_nei',array('begintime'=>$Think['begintime'],'endtime'=>$Think['endtime']));?>" name="Form2" method="post">
    <tr>
    ...
    <tr>
        ...
        <td class="huise_font">
            <select name="unid" id="unid">
				<option value='0' <?php if($paygame["id"] == 0): ?>selected='selected'<?php endif; ?>>充值站点</option>
			  
                <?php if(is_array($hunuser)): $i = 0; $__LIST__ = $hunuser;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["unid"]); ?>' <?php if($vo["unid"] == $get['unid']): ?>selected='selected'<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
...
<thead>
  <tr>
    ...
    <th class="table-textcenter">对应用户</th>
