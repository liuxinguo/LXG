...
var Sc = function () {
    ...
    function getFirstCharge(){
        if (SERVER != 0) {
           ...
           $.ajax({
               type : "get",
               url : "/welfare/get_firstcharge",
               cache : false,
               data : { server_id: SERVER, game_id: gameId },
               dataType : "json",
...
var Cz = function () {
    ...
    $(document).on("click",".tipbox-ingot .sure-to-get", function (e) {
		...
		// alert("尊敬的用户，您好！本功能正在升级中，预计1月31日12:00前开通，给您带来不便敬请谅解，谢谢！");
           $.ajax({
               type : "get",
               url : "/welfare/sure_to_get",
               cache : false,
               data : { gift_type: type, record_id: recordId,server_id: SERVER, game_id: gameId },
               dataType : "json",
               success : function (data) {
...
var Lb = function () {
    ...
    $("#lb-get-gift").click(function () {
        if (SERVER != 0 && GIFT != 0) {
            ...
            $.ajax({
                type : "get",
                url : "/welfare/get_card",
                cache : false,
                data : { server_id: SERVER, card_id: GIFT,game_id: gameId },
                dataType : "json",
                success : function (data) {
                    if (data.status == "1") {
                        //没登陆
