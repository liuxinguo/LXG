...
function inpText(sel){
	sel.filter(':text').each(function(){
		$(this).data( "txt",$(this).val() );
		}).focus(function(){
			if ( $(this).val() === $(this).data("txt") ) $(this).val("");
		}).blur(function(){
			if ($(this).val() === "" ) $(this).val( $(this).data("txt") );
	});
}
function Msg(a) {
	var Btn =$('#loginBtn');
	if($('#lg-tips').length){
		$('#lg-tips').fadeIn(100).html(a);
	}else{
		Btn.append("<div id='lg-tips' class='lg-tips'>"+a+"</div>");
	}
}
function fixWindow(obj){
	var a =$(obj);
	if(a.length){
		var _top =a.offset().top;
...
function swSlide(obj,id){
	if(id && obj){
		$.getJSON("http://www.602.com/index.php?m=poster&c=index&a=show_poster&id="+id+"&type=JSON&jsonpCallback=?",function(result){
			var list = result.Data;
			var str ="<div id='slideBox'>";
				str+="<div class='hd'><ul>";
			for (var i in list){
				str += "<li>"+(++i)+"</li>";
			}
			...
			str += "</ul></div></div>";
			$(obj).html(str).slide({mainCell:".bd ul",effect:"fade",autoPlay:true});
		});
	}
}
function TgSet(){
...
(function showFlash(){
	$(".hd-banner embed").css({"width":"100%","height":"100%"})
	setTimeout(function(){
		$(".hd-banner").css({
			"width":"750px",
			"height":"80px",
