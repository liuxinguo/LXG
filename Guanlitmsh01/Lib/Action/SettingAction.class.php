  ...
  /**
    ...
    */
    ...
    public function pay_game_record(){
       ...
       $condition[C('DB_PREFIX').'pay_togame.cid']     = array('exp',' is NULL');
	 //echo "<pre>";
  // print_r( $condition);
	// echo "</pre>";
        //实例化类
...
//        die;
    ...
    public function pay_game_reissue(){
       ...
       $condition['pay_platform'] = array('exp',' is NULL');  
        //实例化类
        $ShowPage = new ShowPage;
        $ShowPage->PageSize = $this->pagecount; //每页显示记录
        $ShowPage->Total = $model->where($condition)->order(C('DB_PREFIX').'pay_togame.id desc')->count(); //取得总记录数
    ...
    */   
    public function member(){
       ...
       if($get['mobile']){
        ...
        $ShowPage = new ShowPage;
        $ShowPage->PageSize = $this->pagecount; //每页显示记录
        $ShowPage->Total = $model->where($condition)->order('uid desc')->count(); //取得总记录数
...
//        die;
    ...
    */  
    public function register_log(){
        ...
        //查询推广ID
        $mid = M('register_log')->where("mid <> ''")->group('mid')->field('mid')->select();
        $this->assign('mid',$mid);
       ...
       //查询游戏
       if($get['gid']){
           $condition['gid'] = array('eq',$get['gid']);
            //读取充值服务器列表
            $model = M('game_server');
            $payserver= $model->where("gid={$get['gid']}")->order('sid desc')->select();
            $this->assign('payserver',$payserver);
       ...
       //查询服务器
       if($get['sid']){
           $condition['sid'] = array('eq',$get['sid']);
       }
       ...
       //注册IP
       if($get['regip']){
           $condition['regip'] = array('eq',$get['regip']);
       }
       ...
       //备注
       if($get['remark']){
           $condition['remark'] = array('like','%'.$get['remark'].'%');
       }
...
//        die;
    ...
    public function login_log(){
        ...
        //查询推广ID
        $mid = M('login_log')->where("mid <> ''")->group('mid')->field('mid')->select();
        $this->assign('mid',$mid);
       ...
       $get['endtime'] = strtotime($get['endtime'] . ' 23:59:59');
        ...
        if($get['begintime'] > $get['endtime']){
           die(ShowMsg('结束时间不能小于开始时间！', 'javascript:history.back();'));
            ...
            $condition['logintime'] = array('between',array($get['begintime'],$get['endtime']));
        }
       ...
       //注册IP
       if($get['loginip']){
           $condition['loginip'] = array('eq',$get['loginip']);
       }
