...
</style></head><body class="home_body"><div id="Container"><div id="header" class="header"></div><!--end header--><div id="Wrapper"><div id="left" class="Left"></div><!--end Left--><div id="right" class="Right"><div class="content"><iframe id="myframe" name="myframe" src="<?php echo U('Index/home');?>" frameborder="0" scrolling="no" onload="this.height=100" width="100%" ></iframe></div></div><!--end Right--></div><!--end Wrapper--></div><!--end Container--><?php if($cate == 1): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/admin');?>");
</script><?php endif; if($cate == 2): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/yunying');?>");
</script><?php endif; if($cate == 3): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/caiwu');?>");
</script><?php endif; if($cate == 4): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/shangwu');?>");
</script><?php endif; if($cate == 5): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/meijie');?>");
</script><?php endif; if($cate == 6): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/kefu');?>");
</script><?php endif; if($cate == 7): ?><script type="text/javascript">	$("#header").load("<?php echo U('Index/header');?>");
     $("#left").load("<?php echo U('Index/gonghui');?>");
</script><?php endif; ?><!--弹出窗口--><div class="modal hide" id="myModal" ><div class="modal-header"><button type="button" class="close" data-dismiss="modal" id="picgo2">×</button><h3>裁剪缩略图</h3></div><div class="modal-body"><div style="border-bottom:#f4f4f4 solid 1px;
